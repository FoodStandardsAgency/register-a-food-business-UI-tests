...
Business Type In Validation
    ...
    Scenario: testing business type in happy path
        Given I open the url "/business-type"
        When I set "Restaurant" to the inputfield "businessTypeIn.search"
        And I select the 1st option for element "businessTypeIn.autocomplete"
    ...
    Scenario: testing business type in something but not selecting something
        ...
        And I expect that element "businessTypeIn.error" contains the text "You must select a valid business type before continuing"
