...
This repository and configuration file are edited from the [webdriver.io cucumber boilerplate](https://github.com/webdriverio/cucumber-boilerplate/blob/master/wdio.conf.ts). The config file can be found [here](../../wdio.conf.cjs).
...
This pulls in the browserstack username and password from the environment variables, which need to be generated in your own account and pasted into your .env

## Suites

Test suites are used to group together feature files for organisation purposes, and to help run the whole test suite together with parallel tests and without timeouts.
You can find more information on suites and running specific tests/suites [here](http://v4.webdriver.io/guide/testrunner/organizesuite.html)
